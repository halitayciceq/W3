...
var OpsTask = (function() {
    ...
    var config = {
        ...
        company_id: 0,
        branch_id: 0,
        ajax_url: '/V16/order/form/ajax_ops_task.cfm'
    };
    ...
    function loadList() {
        ...
        .done(function(response) {
            console.log('[OpsTask] List response:', response);
            if (response.success) {
                state.tasks = response.data || [];
                renderList(state.tasks);
            } else {
                console.error('[OpsTask] List error:', response.message);
                // Boş liste göster, hata varsa da modal açılabilsin
                renderList([]);
            }
        })
        .fail(function(xhr, status, error) {
            console.error('[OpsTask] AJAX error:', status, error, xhr.responseText);
            // SP veya tablo yoksa boş liste göster
            renderList([]);
        });
    }
    ...
    function openModal(taskId) {
        ...
        $('#ops_task_steps_container').empty();
        
        // Percent alanını sıfırla
        $('#ops_percent_complete').prop('readonly', false).val(0);
        $('#ops_percent_readonly_hint').hide();
        
        // Matris alanlarını sıfırla
        $('#ops_has_matrix').prop('checked', false);
        $('#ops_matrix_template_id').hide().val('');
        
        if (taskId) {
            // Mevcut görevi yükle
            loadTask(taskId);
        } else {
            console.log('[OpsTask] Opening new task modal');
            modal.modal('show');
        }
    }
    ...
    function saveTask() {
        ...
        .done(function(response) {
            console.log('[OpsTask] Save response:', response);
            if (response.success) {
                $('#modal_ops_task').modal('hide');
                loadList();
                showSuccess(response.message || 'Görev kaydedildi');
                
                // Adımları kaydet (varsa)
                if (response.data && response.data.task_id && state.currentTaskId > 0) {
                    saveSteps(response.data.task_id);
                }
            } else {
                showError('Kayıt hatası: ' + (response.message || 'Bilinmeyen hata'));
            }
        })
        .fail(function(xhr, status, error) {
            console.error('[OpsTask] Save AJAX error:', status, error, xhr.responseText);
