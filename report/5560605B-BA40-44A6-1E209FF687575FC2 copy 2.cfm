                                                                        ...
                                                                        <div style="overflow-x:auto;padding:0;">
                                                                            <table class="ui-table-list ui-form form_list" border="1" style="width:100%;">
                                                                                ...
                                                                                <tbody>
                                                                                    ...
                                                                                    <cfif taskIdx eq 0>
                                                                                        <tr><td colspan="12" style="text-align:center;padding:12px 0;"><a href="javascript:void(0)" onclick="createFromTemplate(#orderId#)" style="color:##6b7280;text-decoration:none;font-size:11px;"><i class="fa fa-plus" style="margin-right:4px;"></i>Şablondan Oluştur</a></td></tr>
                                                                                    </cfif>
                                                                                </tbody>
                                                                            </table>
                                                                        </div>
...
<script>
    ...
    function createFromTemplate(orderId) {
        // Önce default şablonu bul, sonra doğrudan uygula
        var fd = new FormData();
        ...
        .then(function(resp) {
            if (resp.success && resp.data && resp.data.length > 0) {
                // Default şablonu veya ilk şablonu kullan
                var template = resp.data.find(function(t) { return t.is_default; }) || resp.data[0];
                
                // Şablonu uygula
                var fd2 = new FormData();
                fd2.append('action', 'batch_create');
                fd2.append('ref_type', 'ORDER');
                fd2.append('ref_id', orderId);
                fd2.append('template_id', template.template_id);
                fd2.append('strategy', 'skip_existing');
                
                fetch('/V16/sales/query/ajax_ops_task.cfm', {method: 'POST', body: fd2})
                .then(function(r2) { return r2.json(); })
                .then(function(resp2) {
                    if (resp2.success) {
                        alert('Görevler şablondan oluşturuldu!');
                        location.reload();
                    } else {
                        alert('Hata: ' + (resp2.message || 'Bilinmeyen hata'));
                    }
                });
            } else {
