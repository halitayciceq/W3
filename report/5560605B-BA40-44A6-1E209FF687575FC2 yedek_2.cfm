...
<head>
    ...
    function projectApp() {
        return {
            ...
            filterPriority: '',
            filterStatus: '',
            init() { console.log('Rapor başlatıldı'); }
        }
    }
    </script>
    ...
    <script>
        ...
        function loadOrderTasks(orderId, buttonEl) {
        ...
        function updateOrderTaskPercent(taskId, newPercent, inputEl, isProductionTask) {
            // Üretim Süreci görevi ise uyarı ver
            if (isProductionTask) {
            ...
            // %'ye göre aşamayı belirle
            var newStageId = STAGE_PLANLAMA;
            if (pct === 0) {
                newStageId = STAGE_IS_ATANDI;
            } else if (pct > 0 && pct < 100) {
                newStageId = STAGE_DEVAM;
            } else if (pct === 100) {
                newStageId = STAGE_TAMAMLANDI;
            }
                ...
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try {
                        ...
                        if (resp.success || resp.SUCCESS) {
                            console.log('Task % updated:', taskId, pct);
                            ...
                            console.error('Task update failed:', resp.message || resp.MESSAGE);
                        }
                    } catch(e) {
        ...
        function updateOrderTaskStage(taskId, stageId, selectEl, isProductionTask) {
            ...
            // Aşamaya göre %'yi belirle
            var newPercent = 0;
            if (parseInt(stageId) === STAGE_TAMAMLANDI || parseInt(stageId) === STAGE_ONAYLANDI) {
                newPercent = 100;
            } else if (parseInt(stageId) === STAGE_DEVAM) {
                // Mevcut değeri koru, değiştirme
                newPercent = -1; // -1 = değiştirme
            }
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try {
                        var resp = JSON.parse(xhr.responseText);
                        if (resp.success || resp.SUCCESS) {
                            console.log('Task stage updated:', taskId, stageId);
                            // % input'unu güncelle (Tamamlandı seçildiyse)
                            if (newPercent >= 0) {
                                var row = selectEl.closest('tr');
                                if (row) {
                                    var pctInput = row.querySelector('input[type="number"]');
                                    if (pctInput) {
                                        pctInput.value = newPercent;
                                    }
                                }
                            }
                        } else {
                            console.error('Task stage update failed:', resp.message || resp.MESSAGE);
                        }
                    } catch(e) {
                        console.error('JSON parse error:', e);
                    }
            ...
            if (newPercent >= 0) {
                sendData += '&percent_complete=' + newPercent;
            }
            xhr.send(sendData);
        }
        
        // Sipariş Task Belge Modal
        ...
        function showTaskFormModal(url) {
            ...
            fetch(url).then(function(r){ return r.text(); }).then(function(html){
                ...
                setTimeout(function(){
                    ...
                    // X butonuna da event listener ekle
                    var closeBtn = body.querySelector('.close, [data-dismiss="modal"], .btn-close');
        ...
        // Sipariş Task Sil
        function deleteOrderTask(taskId, orderId) {
                ...
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try {
                        ...
                        if (resp.success || resp.SUCCESS) {
                            ...
                            alert('Silme hatası: ' + (resp.message || resp.MESSAGE));
                        }
                    } catch(e) {
                        console.error('JSON parse error:', e);
                    }
        ...
        // Sipariş Task Matris Modal - Overlay ile aç
...
<!--- Task Aşamaları (Proje modülü ile aynı - hardcoded) --->
...
<cftry>
    ...
    <!--- Sipariş bazlı task map oluştur (% hesaplama için) --->
    <cfset orderTasksMap = {}>
    <cfset orderPercentMap = {}>
    <cfloop query="GET_ORDER_TASKS">
        <cfset ordKey = toString(ORDER_ID)>
        <cfif NOT structKeyExists(orderTasksMap, ordKey)>
            <cfset orderTasksMap[ordKey] = []>
        </cfif>
        <cfset arrayAppend(orderTasksMap[ordKey], {taskId: TASK_ID, percentComplete: int(PERCENT_COMPLETE)})>
    </cfloop>
    <!--- Her sipariş için ortalama % hesapla --->
    <cfloop collection="#orderTasksMap#" item="ordKey">
        <cfset tasks = orderTasksMap[ordKey]>
        <cfset totalPct = 0>
        <cfloop array="#tasks#" index="t">
            <cfset totalPct = totalPct + t.percentComplete>
        </cfloop>
        <cfset orderPercentMap[ordKey] = arrayLen(tasks) gt 0 ? int(totalPct / arrayLen(tasks)) : 0>
    </cfloop>
    
    <!--- Proje bazında sipariş %'lerinin ortalaması --->
    <cfset projectOrderPercentMap = {}>
    ...
    <cfloop query="GET_PROJECT_ORDERS">
        <cfif NOT structKeyExists(processedOrders, ORDER_ID)>
            ...
            <cfset orderObj = {
                ...
                "percentComplete": structKeyExists(orderPercentMap, toString(ORDER_ID)) ? orderPercentMap[toString(ORDER_ID)] : 0
            }>
...
<!--- Proje bazında sipariş %'lerinin ortalamasını hesapla --->
    <cfloop collection="#projectOrdersMap#" item="projKey">
        <cfset orders = projectOrdersMap[projKey]>
        <cfset totalPct = 0>
        <cfloop array="#orders#" index="ord">
            <cfset totalPct = totalPct + ord.percentComplete>
        </cfloop>
        <cfset projectOrderPercentMap[projKey] = arrayLen(orders) gt 0 ? int(totalPct / arrayLen(orders)) : 0>
    </cfloop>
<cfcatch type="any">
    <cfset projectOrdersMap = {}>
    <cfset stageMap = {}>
...
<div x-data="{ loading: false, searchQuery: '', filterCategory: '', filterPriority: '', filterStatus: '' }" x-init="console.log('Rapor başlatıldı')" class="w-full pt-10">
                    ...
                    <cfif GET_PROJECT_OPERATIONS.recordcount gt 0>
                        <cfoutput query="GET_PROJECT_OPERATIONS">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <!-- Tamamlanma (Sipariş Task Ortalaması) -->
                                <cfset projPct = structKeyExists(projectOrderPercentMap, toString(PROJECT_ID)) ? projectOrderPercentMap[toString(PROJECT_ID)] : int(COMPLETION_RATE)>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <div class="flex items-center" id="project-progress-#PROJECT_ID#">
                                        <div class="w-full bg-gray-200 rounded-full h-2 mr-2">
                                            <div id="project-bar-#PROJECT_ID#" class="h-2 rounded-full <cfif projPct gte 100>bg-green-500<cfelseif projPct gte 75>bg-blue-500<cfelseif projPct gte 50>bg-yellow-500<cfelse>bg-red-500</cfif>" 
                                                 style="width: #projPct#%"></div>
                                        </div>
                                        <span id="project-percent-#PROJECT_ID#" class="text-sm font-medium text-gray-900">#projPct#%</span>
                                    </div>
                                </td>
                            </tr>
                            
                            <!-- Expandable Details Row - Full Width Accordion Layout -->
                            <tr x-show="showDetails" class="bg-gray-50">
                                <td colspan="12" class="p-0">
                                    <div class="w-full" style="display:flex; flex-direction:column; gap:0;">
                                                    ...
                                                    <table class="w-full text-xs min-w-[1400px]">
                                                        <thead>
                                                            <tr class="text-gray-700 bg-gray-100">
                                                                <th class="py-2 px-2 text-center font-semibold w-8"></th>
                                                        ...
                                                        <tbody>
                                                            <template x-for="order in orders" :key="order.orderId">
                                                                <tr class="border-t border-gray-200 hover:bg-gray-50">
                                                                    <td class="py-2 px-2 text-center">
                                                                        <button onclick="loadOrderTasks(this.getAttribute('data-order-id'), this)" 
                                                            ...
                                                            <!--- Sipariş Task Listesi (Görevler tasarımı ile birebir aynı) --->
                                                            <cfloop query="GET_PROJECT_ORDERS">
                                                                            ...
                                                                            <table class="ui-table-list ui-form form_list" border="1" style="width:100%;">
                                                                                <thead>
                                                                                    <tr>
                                                                                        ...
                                                                                        <th width="20"><i class="fa fa-minus" title="Sil"></i></th>
                                                                                    </tr>
                                                                                </thead>
                                                                                <tbody>
                                                                                    <cfset taskIdx = 0>
                                                                                    <cfloop query="GET_ORDER_TASKS">
                                                                                        <cfif GET_ORDER_TASKS.ORDER_ID eq orderId>
                                                                                            ...
                                                                                            <tr class="ops-task-row" draggable="true" data-task-id="#TASK_ID#">
                                                                                                ...
                                                                                                <td style="text-align:center"><cfif isProductionTask><input type="number" min="0" max="100" value="#int(PERCENT_COMPLETE)#" id="order-task-pct-#TASK_ID#" style="width:50px;padding:4px 6px;text-align:center;border:1px solid ##d1d5db;border-radius:4px;background:##f3f4f6;cursor:not-allowed;font-size:12px;" readonly onfocus="alert('Üretim Süreci yüzdesi sadece Üretim Matrisi üzerinden yönetilebilir.');this.blur();"><cfelse><input type="number" min="0" max="100" value="#int(PERCENT_COMPLETE)#" id="order-task-pct-#TASK_ID#" style="width:50px;padding:4px 6px;text-align:center;border:1px solid ##14b8a6;border-radius:4px;background:##fff;font-size:12px;" onblur="updateOrderTaskPercent(#TASK_ID#, this.value, this, false)"></cfif></td>
                                                                                                ...
                                                                                                <td style="text-align:center"><a href="javascript:void(0)" onclick="deleteOrderTask(#TASK_ID#, #orderId#)" title="Sil"><i class="fa fa-minus" style="color:grey"></i></a></td>
                                                                                            </tr>
                                                                                        </cfif>
                                                                                    </cfloop>
                                                                                    <cfif taskIdx eq 0>
                                                                                        <tr><td colspan="13" style="text-align:center;padding:20px;">Görev bulunamadı</td></tr>
                                                                                    </cfif>
                                                                                </tbody>
                                                                            </table>
                                                                        </div>
                                        ...
                                        <!--- ACCORDION 2: GÖREVLER (Mavi Ton) - Şimdilik Gizli --->
                                        <div x-data="{ open: false }" class="border-b border-gray-200" style="display:none;">
