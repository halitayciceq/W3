...
<!-- WC_TASKS_BEGIN - Sipariş Operasyon Görevleri v3 (2026-01-23) -->
<cfoutput>
<script>
(function(){
'use strict';
...
// Config
var CFG = {
	orderId: #val(attributes.order_id)#,
...
var state = { open: false, loaded: false, loading: false };

// Portal: Modal'ı body sonuna ekle
function createModal(){
	if(document.getElementById('wc_tasks_root')) return;
	
	var root = document.createElement('div');
	root.id = 'wc_tasks_root';
	root.innerHTML = 
		'<style>' +
		'##wc_tasks_overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.6);z-index:2147483640;display:none;}' +
		'##wc_tasks_modal{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:900px;max-width:94vw;max-height:88vh;background:##fff;border-radius:8px;box-shadow:0 10px 40px rgba(0,0,0,0.5);z-index:2147483641;display:none;overflow:hidden;}' +
		'##wc_tasks_header{background:linear-gradient(135deg,##3498db,##2980b9);color:##fff;padding:14px 18px;font-size:15px;font-weight:600;}' +
		'##wc_tasks_header .close-btn{float:right;background:none;border:none;color:##fff;font-size:24px;cursor:pointer;line-height:1;opacity:0.8;}' +
		'##wc_tasks_header .close-btn:hover{opacity:1;}' +
		'##wc_tasks_body{max-height:calc(88vh - 50px);overflow:auto;background:##f8f9fa;}' +
		'</style>' +
		'<div id="wc_tasks_overlay" onclick="WC_TASKS_close()"></div>' +
		'<div id="wc_tasks_modal">' +
		'<div id="wc_tasks_header"><i class="fa fa-tasks"></i> Görevler (Operasyonlar) <span style="opacity:0.8;font-weight:normal;margin-left:8px;">' + CFG.orderNo + '</span><button class="close-btn" onclick="WC_TASKS_close()">&times;</button></div>' +
		'<div id="wc_tasks_body"><div style="padding:60px;text-align:center;"><i class="fa fa-spinner fa-spin fa-2x"></i><br><br>Yükleniyor...</div></div>' +
		'</div>';
...
// Open
window.WC_TASKS_open = function(){
	createModal();
	document.getElementById('wc_tasks_overlay').style.display = 'block';
	document.getElementById('wc_tasks_modal').style.display = 'block';
	document.body.style.overflow = 'hidden';
	state.open = true;
	if(!state.loaded && !state.loading) loadContent();
};

// Close  
window.WC_TASKS_close = function(){
	var overlay = document.getElementById('wc_tasks_overlay');
	var modal = document.getElementById('wc_tasks_modal');
...
function loadContent(){
	var body = document.getElementById('wc_tasks_body');
	if(!body) return;
	
	state.loading = true;
	body.innerHTML = '<div style="padding:60px;text-align:center;"><i class="fa fa-spinner fa-spin fa-2x"></i><br><br>Yükleniyor...</div>';
	
	var url = CFG.listUrl + '?ref_type=ORDER&ref_id=' + CFG.orderId + '&company_id=' + CFG.companyId + '&_=' + Date.now();
	...
	xhr.onload = function(){
		...
		if(xhr.status === 200){
			...
			scripts.forEach(function(s){
				if(s.textContent) try { eval(s.textContent); } catch(e){ console.error(e); }
			});
			setTimeout(function(){
				if(typeof window.OpsTask !== 'undefined' && typeof window.OpsTask.init === 'function'){
					window.OpsTask.init();
				}
			}, 50);
		} else {
			body.innerHTML = '<div style="padding:40px;text-align:center;color:##c00;">Hata: ' + xhr.status + '</div>';
		}
	};
	
	xhr.onerror = function(){
		state.loading = false;
		body.innerHTML = '<div style="padding:40px;text-align:center;color:##c00;">Bağlantı Hatası</div>';
	};
...
function injectMenu(){
	...
	menus.forEach(function(ul){
		if((ul.innerHTML.indexOf('Fatura') > -1 || ul.innerHTML.indexOf('Ödeme') > -1) && !ul.querySelector('.wc-tasks-menu-item')){
			var li = document.createElement('li');
			li.className = 'wc-tasks-menu-item';
			li.innerHTML = '<a href="javascript:WC_TASKS_open();" style="cursor:pointer;"><i class="fa fa-tasks" style="color:##3498db;margin-right:5px;"></i>Görevler</a>';
...
<script type="text/javascript">
	...
	function kontrol()
	{
		var order_reserved = wrk_safe_query("order_reserved",'dsn3',0,<cfoutput>#attributes.order_id#</cfoutput>);
		...
		{
			alert("<cfoutput>#getLang('','Cari Hesap Seçmelisiniz',45308)#</cfoutput>");
			return false;
		}
		if((form_basket.order_employee_id.value.length == 0) || (form_basket.order_employee.value.length == 0))
		{
			alert("<cfoutput>#getLang('','Satış Yapan',38549)#</cfoutput> !");
			return false;
		}
		if (form_basket.deliverdate.value.length == 0)
		{
			alert("<cfoutput>#getLang('','Teslim Tarihi Girmelisiniz',46856)#</cfoutput> !");
			return false;
		}
		x = (500 - document.form_basket.ship_address.value.length);
		if ( x < 0 )
		{
			alert ("<cfoutput>#getLang('','Sevk Adresi',57248)#</cfoutput><cfoutput>#getLang('','Alanındaki Fazla Karakter Sayısı',58210)#</cfoutput>"+ ((-1) * x));
			return false;
		}
	...
	function orderControl(orderNumber) {
		alert("<cfoutput>#getLang('','Bu sipariş daha önce irsaliyeye bağlanmmış',51191)#</cfoutput>");
