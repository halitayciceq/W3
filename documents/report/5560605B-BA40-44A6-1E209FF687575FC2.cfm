...
<head>
    ...
    <style>
        ...
        .task-th { background-color: #d6dce5 !important; color: #333 !important; border: 1px solid #c8ced7 !important; font-weight: 600; padding: 8px 6px; text-align: center; white-space: nowrap; font-size: 12px; }
        .task-table tbody td { border: 1px solid #e5e7eb; padding: 6px 5px; font-size: 12px; }
    </style>
</head>
...
<cftry>
    ...
    <!--- Sipariş Tamamlanma Oranları (task ortalaması) --->
    ...
    <cfloop query="GET_PROJECT_ORDERS">
        <cfif NOT structKeyExists(processedOrders, ORDER_ID)>
            ...
            <cfset orderObj = {
                ...
                "totalAmount": (len(MONEY_NAME) AND MONEY_NAME NEQ 'TL' AND MONEY_NAME NEQ 'TRY' AND val(OTHER_MONEY_VALUE) GT 0) ? numberFormat(OTHER_MONEY_VALUE, ',.00') : numberFormat(NETTOTAL, ',.00'),
                "totalAmountRaw": (len(MONEY_NAME) AND MONEY_NAME NEQ 'TL' AND MONEY_NAME NEQ 'TRY' AND val(OTHER_MONEY_VALUE) GT 0) ? val(OTHER_MONEY_VALUE) : val(NETTOTAL),
                "currency": (len(MONEY_NAME) AND MONEY_NAME NEQ 'TL' AND MONEY_NAME NEQ 'TRY') ? MONEY_NAME : 'TL',
                "stage": stageName,
...
<cftry>
    <cfquery name="GET_ORDER_TASKS" datasource="#DSN#">
        SELECT 
            T.TASK_ID, T.TASK_HEAD, T.REF_ID AS ORDER_ID, T.STATUS_ID,
            T.PERCENT_COMPLETE, T.DEADLINE, T.ESTIMATED_MINUTES,
            T.ACTUAL_MINUTES, T.HAS_MATRIX, T.ASSIGNED_EMP_ID,
...
<cfcatch>
    <cfset GET_ORDER_TASKS = queryNew("TASK_ID,TASK_HEAD,ORDER_ID,STATUS_ID,PERCENT_COMPLETE,DEADLINE,ESTIMATED_MINUTES,ACTUAL_MINUTES,HAS_MATRIX,ASSIGNED_EMP_ID,EMPLOYEE_NAME,EMPLOYEE_PHOTO")>
</cfcatch>
</cftry>
...
<cfset queryAddRow(GET_TASK_STAGES, 5)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_ID", 2358, 1)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_HEAD", "Planlama", 1)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_ID", 2359, 2)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_HEAD", "İş Atandı", 2)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_ID", 2361, 3)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_HEAD", "Devam Ediyor", 3)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_ID", 2364, 4)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_HEAD", "Tamamlandı", 4)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_ID", 2365, 5)>
<cfset querySetCell(GET_TASK_STAGES, "STATUS_HEAD", "Onaylandı", 5)>
<cfcatch>
    <cfset GET_TASK_STAGES = queryNew("STATUS_ID,STATUS_HEAD")>
</cfcatch>
</cftry>
                        ...
                        <cfoutput query="GET_PROJECT_OPERATIONS">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <td class="px-4 py-4 text-sm text-gray-900 max-w-xs">
                                    <input type="text" value="#htmlEditFormat(PROJECT_HEAD)#" style="border:1px solid ##d1d5db;border-radius:4px;padding:4px 8px;font-size:13px;width:100%;background:##fff;" onblur="updateProjectField(#PROJECT_ID#,'project_head',this.value,this)">
                                </td>
                                ...
                                <!-- Kategori -->
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <select style="border:1px solid ##d1d5db;border-radius:4px;padding:4px 6px;font-size:12px;background:##fff;" onchange="updateProjectField(#PROJECT_ID#,'process_cat',this.value,this)">
                                        <cfloop query="GET_CATEGORIES"><option value="#GET_CATEGORIES.MAIN_PROCESS_CAT_ID#" <cfif GET_PROJECT_OPERATIONS.PROCESS_CAT eq GET_CATEGORIES.MAIN_PROCESS_CAT_ID>selected</cfif>>#GET_CATEGORIES.MAIN_PROCESS_CAT#</option></cfloop>
                                    </select>
                                </td>
                                
                                <!-- Öncelik -->
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <select style="border:1px solid ##d1d5db;border-radius:4px;padding:4px 6px;font-size:12px;background:##fff;" onchange="updateProjectField(#PROJECT_ID#,'priority',this.value,this)">
                                        <cfloop query="GET_PRIORITIES"><option value="#GET_PRIORITIES.PRIORITY_ID#" <cfif GET_PROJECT_OPERATIONS.PRO_PRIORITY_ID eq GET_PRIORITIES.PRIORITY_ID>selected</cfif>>#GET_PRIORITIES.PRIORITY#</option></cfloop>
                                    </select>
                                </td>
                                
                                <!-- Başlangıç -->
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <input type="date" value="#dateformat(TARGET_START, 'yyyy-mm-dd')#" style="border:1px solid ##d1d5db;border-radius:4px;padding:4px 6px;font-size:12px;background:##fff;" onchange="updateProjectField(#PROJECT_ID#,'target_start',this.value,this)">
                                </td>
                                
                                <!-- Bitiş -->
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <input type="date" value="#dateformat(TARGET_FINISH, 'yyyy-mm-dd')#" style="border:1px solid ##d1d5db;border-radius:4px;padding:4px 6px;font-size:12px;background:##fff;" onchange="updateProjectField(#PROJECT_ID#,'target_finish',this.value,this)">
                                </td>
                                                ...
                                                <div class="p-3 overflow-x-auto">
                                                    ...
                                                    <table class="w-full text-xs min-w-[1400px]">
                                                        ...
                                                        <tbody>
                                                                ...
                                                                <tr class="border-t border-gray-200 hover:bg-gray-50">
                                                                    ...
                                                                    <td class="py-2 px-2 text-center">
                                                                        <div class="flex items-center justify-center gap-1">
                                                                            ...
                                                                            <span class="text-xs text-gray-500" x-text="(order.completionRate || 0) + '%'"></span>
                                                                        </div>
                                                                    </td>
                                                            ...
                                                            <!--- Sipariş Task Listesi --->
                                                            <cfloop query="GET_PROJECT_ORDERS">
                                                                <cfset orderId = GET_PROJECT_ORDERS.ORDER_ID>
                                                                <tr id="order-tasks-#orderId#" style="display:none;" class="bg-gray-50">
                                                                    <td colspan="14" style="padding:0;">
                                                                        <div style="overflow-x:auto;padding:0;">
                                                                            <table class="task-table" style="width:100%;border-collapse:collapse;border-left:none;border-right:none;table-layout:fixed;">
                                                                                <thead>
                                                                                    <tr>
                                                                                        <th width="16" class="task-th" style="border-left:none;"><i class="fa fa-bars" style="font-size:10px" title="Sürükle-Bırak"></i></th>
                                                                                        <th width="30" class="task-th">A</th>
                                                                                        <th width="220" class="task-th" style="text-align:left;">Başlık</th>
                                                                                        <th width="90" class="task-th">Aşama</th>
                                                                                        <th width="80" class="task-th">Termin</th>
                                                                                        <th width="55" class="task-th"><i class="fa fa-clock-o" style="font-size:12px;margin-right:4px;vertical-align:middle;"></i>Öng.</th>
                                                                                        <th width="55" class="task-th"><i class="fa fa-clock-o" style="font-size:12px;margin-right:4px;vertical-align:middle;"></i>Harc.</th>
                                                                                        <th width="40" class="task-th">%</th>
                                                                                        <th width="20" class="task-th"><i class="fa fa-pencil" title="Düzenle"></i></th>
                                                                                        <th width="20" class="task-th"><i class="fa fa-file" title="Dosya"></i></th>
                                                                                        <th width="20" class="task-th"><i class="fa fa-th" title="Matris"></i></th>
                                                                                        <th width="20" class="task-th" style="border-right:none;"><i class="fa fa-plus" title="Ekle"></i></th>
                                                                                    </tr>
                                                                                </thead>
                                                                                <tbody>
                                                                                    <cfset taskIdx = 0>
                                                                                    <cfloop query="GET_ORDER_TASKS">
                                                                                        <cfif GET_ORDER_TASKS.ORDER_ID eq orderId>
                                                                                            ...
                                                                                            <tr class="ops-task-row" data-task-id="#TASK_ID#">
                                                                                                ...
                                                                                                <td style="text-align:center"><span class="time-input-wrap" data-task="#TASK_ID#" data-field="estimated_minutes"><input type="number" min="0" max="999" value="#int(val(ESTIMATED_MINUTES)/60)#" class="time-h" onblur="saveTimeFromInputs(this)" <cfif NOT canEditTask>readonly style="background:##f3f4f6;cursor:not-allowed;"</cfif>><span class="time-sep">:</span><input type="number" min="0" max="59" value="#val(ESTIMATED_MINUTES) mod 60#" class="time-m" onblur="saveTimeFromInputs(this)" <cfif NOT canEditTask>readonly style="background:##f3f4f6;cursor:not-allowed;"</cfif>></span></td>
                                                                                                <td style="text-align:center"><span class="time-input-wrap" data-task="#TASK_ID#" data-field="actual_minutes"><input type="number" min="0" max="999" value="#int(val(ACTUAL_MINUTES)/60)#" class="time-h" onblur="saveTimeFromInputs(this)" <cfif NOT canEditTask>readonly style="background:##f3f4f6;cursor:not-allowed;"</cfif>><span class="time-sep">:</span><input type="number" min="0" max="59" value="#val(ACTUAL_MINUTES) mod 60#" class="time-m" onblur="saveTimeFromInputs(this)" <cfif NOT canEditTask>readonly style="background:##f3f4f6;cursor:not-allowed;"</cfif>></span></td>
                                                                                                <td style="text-align:center"><cfif isProductionTask OR NOT canEditTask><input type="number" min="0" max="100" value="#int(PERCENT_COMPLETE)#" style="width:50px;padding:4px 6px;text-align:center;border:1px solid ##d1d5db;border-radius:4px;background:##f3f4f6;cursor:not-allowed;font-size:12px;" readonly><cfelse><input type="number" min="0" max="100" value="#int(PERCENT_COMPLETE)#" style="width:50px;padding:4px 6px;text-align:center;border:1px solid ##14b8a6;border-radius:4px;background:##fff;font-size:12px;" onblur="updateOrderTaskPercent(#TASK_ID#, this.value, this, false)"></cfif></td>
                                                                                                ...
                                                                                                <td style="text-align:center"><a href="javascript:void(0)" onclick="openOrderTaskDocumentModal(#TASK_ID#, '#jsStringFormat(GET_PROJECT_ORDERS.ORDER_NUMBER)#', '#jsStringFormat(GET_ORDER_TASKS.TASK_HEAD)#')" title="Dosya"><i class="fa fa-file" style="color:grey"></i></a></td>
                                                                                    ...
                                                                                    <cfif taskIdx eq 0>
                                                                                        <tr><td colspan="14" style="text-align:center;padding:12px 0;border-left:none;border-right:none;"><a href="javascript:void(0)" onclick="createFromTemplate(#orderId#)" style="color:##6b7280;text-decoration:none;font-size:11px;"><i class="fa fa-plus" style="margin-right:4px;"></i>Sablondan Olustur</a></td></tr>
                                                                                    </cfif>
                                                                                </tbody>
                                                                            </table>
                                                                        </div>
                                                            ...
                                                            <tr x-show="orders.length === 0">
                                                                <td colspan="14" class="py-4 text-center text-gray-400">Kayit bulunamadi</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
...
<script>
    ...
    function updateChainUI(resp) {
        // Sipariş % güncelle
        var orderPct = resp.order_pct !== undefined ? resp.order_pct : (resp.ORDER_PCT !== undefined ? resp.ORDER_PCT : null);
        if (orderPct !== null && currentOpenOrderId) {
            var orderBtn = document.querySelector('[data-order-id="' + currentOpenOrderId + '"]');
            if (orderBtn) {
        ...
        // Proje % güncelle
        var projectPct = resp.project_pct !== undefined ? resp.project_pct : (resp.PROJECT_PCT !== undefined ? resp.PROJECT_PCT : null);
        var projectId = resp.project_id || resp.PROJECT_ID;
        if (projectPct !== null && projectId) {
            var projBar = document.getElementById('project-bar-' + projectId);
            var projText = document.getElementById('project-percent-' + projectId);
            if (projBar) {
                projBar.style.width = Math.round(projectPct) + '%';
                projBar.className = 'h-2 rounded-full ' + (projectPct >= 100 ? 'bg-green-500' : projectPct >= 75 ? 'bg-blue-500' : projectPct >= 50 ? 'bg-yellow-500' : 'bg-red-500');
            }
    ...
    function toggleOrderTasks(orderId, btn) {
        ...
        allTaskRows.forEach(function(row) { row.style.display = 'none'; });
        ...
        if (currentOpenOrderId === orderId) { currentOpenOrderId = null; return; }
        var taskRow = document.getElementById('order-tasks-' + orderId);
        if (!taskRow) return;
        var orderRow = btn.closest('tr');
        if (orderRow) { orderRow.parentNode.insertBefore(taskRow, orderRow.nextSibling); }
    ...
    function saveTimeFromInputs(inputEl) {
        var wrap = inputEl.closest('.time-input-wrap');
        if (!wrap) return;
        var taskId = wrap.getAttribute('data-task');
        var field = wrap.getAttribute('data-field');
        var hInput = wrap.querySelector('.time-h');
        var mInput = wrap.querySelector('.time-m');
        var h = parseInt(hInput.value) || 0;
        var m = parseInt(mInput.value) || 0;
        if (m > 59) { m = 59; mInput.value = 59; }
        if (h < 0) { h = 0; hInput.value = 0; }
        if (m < 0) { m = 0; mInput.value = 0; }
        var totalMinutes = (h * 60) + m;
        updateTaskTime(taskId, field, totalMinutes, wrap);
    }
    
    function updateTaskTime(taskId, field, value, el) {
        var xhr = new XMLHttpRequest();
            ...
            if (xhr.readyState === 4 && xhr.status === 200) {
                try {
                    ...
                    if (resp.success || resp.SUCCESS) {
                        ...
                        setTimeout(function() { if (el) el.style.borderColor = '##d1d5db'; }, 1000);
                    }
                } catch(e) { console.error(e); }
            }
        };
        xhr.send('action=update_time&task_id=' + taskId + '&field=' + field + '&value=' + (parseInt(value) || 0));
    }
    
    function updateProjectField(projectId, field, value, el) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'V16/sales/query/ajax_ops_task.cfm', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.withCredentials = true;
            ...
            if (xhr.readyState === 4 && xhr.status === 200) {
                try {
                    ...
                    if (resp.success || resp.SUCCESS) {
                        ...
                        setTimeout(function() { if (el) el.style.outline = ''; }, 1000);
                    } else {
                        alert('Hata: ' + (resp.message || 'Güncelleme başarısız'));
                    }
                } catch(e) { console.error(e); }
            }
        };
        xhr.send('action=update_project&project_id=' + projectId + '&field=' + field + '&value=' + encodeURIComponent(value));
    }
    
    function updateOrderTaskPercent(taskId, newPercent, inputEl, isProductionTask) {
        if (isProductionTask) { alert('Üretim Süreci yüzdesi sadece Üretim Matrisi üzerinden yönetilebilir.'); return; }
            ...
            if (xhr.readyState === 4 && xhr.status === 200) {
                try {
                    ...
                    if (resp.success || resp.SUCCESS) {
                        ...
                        if (row) { var stageSelect = row.querySelector('select'); if (stageSelect) stageSelect.value = newStageId; }
                        updateChainUI(resp);
                    }
                } catch(e) { console.error(e); }
            }
    ...
    function updateOrderTaskStage(taskId, stageId, selectEl, isProductionTask) {
        if (isProductionTask) { alert('Üretim Süreci aşaması sadece Üretim Matrisi üzerinden yönetilebilir.'); return; }
        var newPercent = 0;
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                try {
                    var resp = JSON.parse(xhr.responseText);
                    if (resp.success || resp.SUCCESS) {
                        var actualPct = resp.percent_complete !== undefined ? resp.percent_complete : (resp.PERCENT_COMPLETE !== undefined ? resp.PERCENT_COMPLETE : newPercent);
                        var row = selectEl.closest('tr');
                        if (row) {
                            var pctInput = row.querySelector('input[type="number"]');
                            if (pctInput && actualPct >= 0) pctInput.value = Math.round(actualPct);
                        }
                        updateChainUI(resp);
                    }
                } catch(e) { console.error(e); }
        ...
        xhr.send('action=update_stage&task_id=' + taskId + '&status_id=' + stageId + '&percent_complete=' + newPercent);
    }
    
    function createFromTemplate(orderId) {
        var fd = new FormData();
        fd.append('action', 'create_from_template');
        ...
        .then(function(resp) {
            if (resp.success) { alert('Görevler oluşturuldu. Sayfa yenilenecek.'); location.reload(); }
            else { alert('Hata: ' + (resp.message || 'Bilinmeyen hata')); }
    ...
    function openOrderTaskDocumentModal(taskId, orderNumber, taskHead) {
        ...
        iframe.onload = function() {
            try {
                ...
                var docName = orderNumber ? (orderNumber + ' - ' + (taskHead || '')) : (taskHead || '');
                var nameInputs = doc.querySelectorAll('input[name="asset_name"], input[name="doc_name"], input[name="ASSET_NAME"]');
                nameInputs.forEach(function(inp) { if(!inp.value) inp.value = docName; });
                // Tüm text input'ları kontrol et — ilk boş olanı doldur (Doküman Adı)
                if(nameInputs.length === 0) {
                    var allInputs = doc.querySelectorAll('input[type="text"]');
                    for(var i = 0; i < allInputs.length; i++) {
                        var inp = allInputs[i];
                        if(!inp.value && inp.name !== 'doc_no' && inp.name !== 'ASSET_CODE') {
                            inp.value = docName;
                            break;
                        }
                    }
                }
            } catch(e) { console.log('iframe fill error:', e); }
        };
        iframeWrapper.appendChild(iframe);
        modal.appendChild(header);
        modal.appendChild(iframeWrapper);
